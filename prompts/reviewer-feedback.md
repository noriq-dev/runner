...
A finding that is real but NOT this change's to answer for is contested with the task that now tracks it — write the pointer as `task:<key>`, naming the proposal filed through create_tasks (or a task that already exists and covers it). The daemon checks that task is real and hands what it found to the next reviewer beside your answer, so the claim "tracked there" becomes a checkable fact — but a task it cannot find counts as no pointer at all, and proposing follow-up work never excuses anything this change still owes.
