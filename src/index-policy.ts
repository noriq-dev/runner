 ...
 *
 * `cpp`/`ini` (RUN-239) join on MEASURED demand across every Noriq-managed project — Project Nod
...
export const DEFAULT_EXCLUDE_GLOBS: string[] = [
  ...
  '**/node_modules/**',
  // RUN-239: Unreal's compiled asset/level binaries — MEASURED on Project Nod (the one
  // Noriq-managed Unreal project, on Diversion): 3201 `.uasset` + 6 `.umap`, 89% of the repo's 3586
  // total files. Machine-generated by the editor on save, never hand-authored, the same category
  // as a lockfile — unambiguous the same way `node_modules` above is.
  //
  // **What this glob actually buys, measured against `index-scan.ts` rather than assumed**: the
  // exclude/defaultExclude check (`index-scan.ts:501-511`) runs BEFORE the read/binary-sniff step,
  // so a matched `.uasset` never has its bytes read at all — real I/O and CPU avoided on files that
  // are often several MB each, and the file's status reason becomes the accurate, deliberate
  // `excluded-default` (this daemon's OWN policy) rather than `binary` (the same STATUS the file
  // would have gotten anyway, since it would fail the binary sniff regardless — `binary` is never
  // an INCORRECT read, only a more expensive and less informative one). What it does NOT buy: a
  // smaller scan-status collector. `makeShouldDescend` (`index-scan.ts:383-402`) only prunes a
  // whole DIRECTORY for the hard deny list and `vcsIgnored` — an extension glob like this one has
  // no directory-level analogue (soundly: not every file under a directory containing `.uasset`
  // files is itself a `.uasset`), so each matched file still costs its own `pushStatus` call,
  // exactly as `binary` would have. A repo with more `.uasset` files than `MAX_STATUS_RECORDS`
  // (1000) still exhausts the collector — with an accurate reason instead of a misleading one, not
  // with fewer records. Directory-level pruning for `exclude`/`defaultExclude` would close that gap
  // properly; it is a real, larger change to `index-scan.ts`'s matching engine this task does not
  // make (see this task's own final report for the recommendation).
  '**/*.uasset',
  '**/*.umap',
];
