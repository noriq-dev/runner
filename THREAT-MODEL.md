...
| **What is NOT yet true, precisely** | `resolveIndexConfig` is wired into repo discovery, so a malformed `[index]` table is already visible in the daemon's logs today. But `scanRepoForIndex` still has no caller anywhere outside its own test file — nothing in `daemon.ts` or `supervisor.ts` invokes it, on a schedule or otherwise (RUN-214/RUN-222 track the coordinator and its triggers). **Turning `[index].enabled` on today still changes nothing observable**: it validates a config, and — since the RUN-209 follow-up above — a scan run directly against it would now honour `contentMode`, but nothing in the daemon's own pipeline runs that scan yet | (no `Where` — this is the absence of a call site, not a control) |
