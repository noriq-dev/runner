...
- ~~The only secret that crosses the wire is the Noriq OAuth token.~~ Narrower now, and still
  ...
  something actually calls the transport. As of Phase 3 that transport EXISTS as client code
  (`ingest-client.ts`, RUN-220) and the indexer that feeds it produces real generations, but no
  scheduled or run-pipeline caller invokes either: the only thing that indexes today is an operator
  typing `index-repo`, which cannot upload by construction. See "Repository intelligence upload
  (`[index]`)" below for
...
| **What is NOT yet true, precisely** | `resolveIndexConfig` is wired into repo discovery, so a malformed `[index]` table is already visible in the daemon's logs today, and `index-repo` will index a repo end to end on demand. But nothing in `daemon.ts` or `supervisor.ts` invokes the scanner, the indexer, or the ingest client — on a schedule or otherwise (RUN-221/222/223 track the journal, the triggers, and the operator controls). **Turning `[index].enabled` on today still changes nothing the daemon does on its own**: it validates a config and it permits an operator-typed `index-repo` to run without `--force`. Nothing uploads, because nothing calls the upload | (no `Where` — this is the absence of a call site, not a control) |
| **PARTLY IMPLEMENTED (Phase 4, in progress)** | Short-lived, single-purpose ingest capability tokens scoped to one generation's upload — never the operator's own OAuth token, never long-lived — now EXIST as client code: `mintIngestCapability` requests one and `openIngestUpload` spends it, with the token carried in the URL path rather than a header (the server's choice) and a closed refusal vocabulary (`expired`, `wrong-scope`, `disabled`, …) so a rejection is named rather than guessed. What is still only DESIGNED, stated here in the future tense with no `Where` because the code does not exist: staging uploaded batches in object storage before one atomic activation transaction selects a generation as active, only once its counts/hashes/deletions validate (`IndexGenerationManifest` stays queryable as "pending" until then); an upload journal with resume, so a killed daemon mid-upload does not restart a whole generation (RUN-221 — note `index-journal.ts` today records COORDINATOR state, not upload progress); and **capability revocation, so a compromised or stale ingest token can be cut off independent of the operator's own credential — this does not exist in any form, and a minted capability's 15-minute TTL is currently the only thing that ends it** | `src/ingest-client.ts` `openIngestUpload`, `src/client.ts` `mintIngestCapability` |
...
| **Residual risk — a secret pasted into an ordinary file** | The deny list covers known secret-bearing PATHS (`.env`, `id_rsa`, …), never file CONTENT. RUN-218's value floor narrows this but does not close it, and the boundary is worth stating exactly rather than approximately: it withholds a secret-shaped VALUE an adapter extracted from JSON/TOML/markdown, so `token = "ghp_…"` in a committed `config.toml` is withheld — but a `full`-mode file entity carries the file's RAW decoded text with no redaction pass at all, so the same token hardcoded into `src/config.ts` is in the payload exactly as before. Measured, not assumed (RUN-219). Two reasons it stays this way rather than getting a blanket floor: composing the value heuristic over whole source files over-redacts badly (a file merely CONTAINING the literal `sk-` loses its whole content — `src/adjudication.ts` in this very repo does), and a content-scanning secret detector is a different tool with a different false-positive budget than a path list. `index-repo`'s own display floor is separate and applies only to what it prints, never to what a payload would contain. **Phase 4 wires the upload: this is the decision it must make explicitly, not inherit** |
