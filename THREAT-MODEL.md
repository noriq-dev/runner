...
| A retrieved memory or episode (Project Memory's context pack) talks a prompt into doing something the task never asked, or its own gate into a PASS | RUN-228 fetches a task's context pack, RUN-229 verifies every citation against THIS run's own leased worktree (never trusting the server's own verdict), and RUN-231 is the ONE renderer (`memory-render.ts`) that may fold either into a prompt — the same "record now, render later" split `[index]`'s own row above states, closed the same way `[context]` was (RUN-154's pattern, reused rather than reinvented). Every line built from retrieved text carries a fixed `\| ` prefix — containment is structural (a line either starts with the prefix or it does not), never a scan for an injection PHRASE, the standing RUN-189 lesson (shape-nets over adversarial text leak at the next composed mutation). The block states out loud that it cannot change scope, permissions, acceptance criteria, review rules, or verdict; for a JUDGING actor (the inline reviewer, the dispatched verify run, and — since RUN-232 — the plan checker) an attempt to instruct is itself a finding, the identical `[context]` reviewer-audience move. RUN-232 wires the same renderer into the planner and pattern mapper (AUTHOR audience) and adds one sentence to that frame: an execution spec's `lockedDecisions`, and whatever an actor verifies itself by reading the repository, outrank anything quoted here — stated because precedence an agent cannot see is precedence it will not apply, not because the containment mechanism changed. RUN-232 also derives which verified citation paths fall outside a spec's own declared `anticipatedFiles` and records that divergence as a run-transcript milestone for a human to read — never as a second input to predictive locking: `continuationLockScope` (`daemon.ts`) still takes only a `Run` and an `ExecutionSpec`, so nothing this derivation returns can reach a lock. Budgets are independent per audience (16k author / 2k reviewer, the `[context]`/`REVIEWER_CONTEXT_MAX_CHARS` precedent) and a cut is marked, never silent. Ordering matters and is enforced at the template: in both verify templates `{{memory}}` sits ahead of the daemon's own acceptance/verdict instructions, so the daemon's rules are the last word a judging actor reads. **What this is NOT**: isolation. The real floor is the same one every other row here states — `clampPermissionToWorkflow` keeps a judging actor read-only regardless of what any prompt says, the reviewer holds no Noriq credential, and a verdict still only gates, it never merges. A memory whose citations verify as `valid` is not thereby "safe content" either — it is still retrieved prose from an untrusted author (any agent or human who ever called `record_memory`), rendered through the SAME quoted frame as a memory that failed every citation check; verification changes whether an excerpt is demoted to a visible LEAD, never whether its text is quoted. | `memory-render.ts` `renderMemoryEvidence`, `suggestedMemoryPaths`, `citation-verify.ts` `verifyContextPack`, `daemon.ts` `continuationLockScope`, `prompts/build.md`, `prompts/scope.md`, `prompts/verify-agent.md`, `prompts/reviewer.md`, `prompts/planner.md`, `prompts/pattern-mapper.md`, `prompts/plan-checker.md` |
...
| **Prompt injection** | Indexed content is source TEXT, never instructions to an agent, and nothing in RUN-207…209 renders scanned content into any prompt directly — this daemon's OWN indexed bytes reach a model only indirectly, by first becoming a citation's evidence inside a context pack. That path is now built (RUN-228…231, not this document's `[index]` section): `context-pack.ts` fetches, `citation-verify.ts` verifies every citation against the leased worktree, and `memory-render.ts` is the ONE renderer allowed to fold a pack into a prompt, through the same quoted-evidence framing `renderRepoContext`'s reviewer audience established for `[context]` — see the "A retrieved memory or episode…" row in the main table above for the full statement of that boundary. | `memory-render.ts` `renderMemoryEvidence`, `citation-verify.ts` `verifyContextPack` (the row above has the full account) |
