...
| A committed manifest turns the daemon into an arbitrary-file-read primitive | `[context]` (RUN-128/129) names files whose **contents** are inlined into the brief, and `.noriq/project.toml` travels with the repo — so every path in it is untrusted input read on the operator's box. **Confinement binds the OPEN, not a name** (RUN-151): the reader opens first, then requires the re-resolved path to sit inside the re-resolved root *and* to be the same inode (`dev`/`ino`) as the descriptor it holds — then reads from that descriptor and never consults the path again. A symlink out, a symlinked parent directory, or a swap racing the check is refused, and a refusal is **named in the brief** rather than silently dropped (a list that quietly shrinks to nothing is worse than one never configured). Reads are bounded at 4× the budget in bytes and the open is `O_NONBLOCK` (a committed symlink may point at a FIFO, where a blocking open waits forever), so a hostile marker can neither OOM nor hang prompt assembly. **Where it stops**: an attacker who can already write to the checkout as the operator can hardlink or bind-mount an outside file to a genuinely in-repo path — the inodes match because it is the same file, and no fd check distinguishes that from ordinary repo content. That attacker is inside the boundary already. What is defended is the case that travels: a committed marker, on a box the daemon trusts. The repo root is trusted input (the daemon's own `scanRoots` walk); the manifest supplies only the leaf path | `repo-context.ts` `openConfined`, `defaultDocReader` |
| A repo's committed manifest talks its own gate into a PASS | Since RUN-154 the repo's `[context]` also reaches the **verify actor and the inline reviewer** — the actors that decide PASS/FAIL. That is deliberate (a reviewer judging whether a diff fits this repo's conventions was the one actor never told them) and it is why the rendering differs from the author's. Three things carry it: the block is introduced as **quoted evidence, not instructions**, and says outright that it cannot change the review rules, the scope, or the verdict; an attempt to do so is declared **a finding in itself**, which is the one response the attempt cannot want; and the daemon's own verdict instructions are placed **after** it in both templates, so the last word belongs to the side the repo does not write. It is also **bounded** (2k, vs 16k for an author) and **names-only** — no file contents — so a repo cannot crowd out the diff by writing a great deal. A prompt frame is not isolation and is not claimed as one: the real floors are unchanged (`clampPermissionToWorkflow` keeps the actor read-only, the reviewer holds no Noriq credential, and a gate's verdict still only gates — it never merges) | `repo-context.ts` `renderRepoContext` (reviewer audience), `prompts/verify-agent.md`, `prompts/reviewer.md` |
