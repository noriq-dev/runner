...
- Git submodules are opt-in per project through `[sourceControl.submodules]`, with a policy per path: `pinned` (default) populates and refuses any change, `follow` lets a gitlink advance only onto a commit already on its configured upstream target, and `develop` lets agents author commits inside the submodule. Every workspace populates its submodules before setup runs, because `git worktree add` leaves them empty. A dirty submodule is never staged as a gitlink: under `pinned` and `follow` the candidate is refused, and under `develop` Runner commits inside the submodule itself, because workers are forbidden source-control commands and may have no shell. Authored commits are transferred into the parent repository under `refs/noriq/submodule/...` before the parent checkpoint exists, since a worktree's submodule store is deleted with the worktree and a ref inside it would die with the objects. Landing runs submodule-first and gates the parent on it; `develop` is refused in direct mode, which never lands. Failed work is preserved the same way. Nested submodules inside a submodule are not governed individually — the containing submodule's own status covers them. Perforce and Diversion advertise no submodule support, and a submodule-configured project is refused against them.
